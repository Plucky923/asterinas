...
use framev_sock_common::{ConnectionId, FrameVsockFlowKey};

use super::Connected;
use crate::{
    net::socket::framevsock::{addr::VMADDR_CID_HOST, backend},
    prelude::*,
};
...
impl Connected {
    ...
    pub(super) fn compute_vcpu_id(id: &ConnectionId) -> usize {
        let target_cid = if id.peer_addr.cid == VMADDR_CID_HOST {
            id.local_addr.cid
        } else {
            id.peer_addr.cid
        };
        let vcpu_count = backend::vcpu_count_for_cid(target_cid);
        FrameVsockFlowKey::new(id.local_addr, id.peer_addr).preferred_queue(vcpu_count)
    }
}
