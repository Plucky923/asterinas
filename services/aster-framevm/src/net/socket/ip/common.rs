...
use crate::{
    net::{
        iface::{Iface, framev_iface, iter_all_ifaces, loopback_iface},
...
fn get_ephemeral_iface(remote_ip_addr: &IpAddress) -> Arc<Iface> {
    match remote_ip_addr {
        IpAddress::Ipv4(remote_ipv4_addr) => {
            ...
            if let Some(framev_iface) = framev_iface() {
                framev_iface.clone()
            } else {
        ...
        IpAddress::Ipv6(remote_ipv6_addr) => {
            ...
            // Prefer FrameV Ethernet over loopback for external traffic.
            if let Some(framev_iface) = framev_iface()
                && framev_iface.ipv6_addr().is_some()
            {
                return framev_iface.clone();
            }
