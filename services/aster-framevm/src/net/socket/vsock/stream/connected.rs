...
impl ConnectedStream {
    pub(super) fn new(
        ...
        Self {
            bound_port,
            peer_addr,
            transport_generation: transport::current_transport_generation(),
    ...
    pub(super) fn sendmsg(&self, input: &[u8], _flags: SendRecvFlags) -> Result<usize> {
        ...
        self.observe_transport_generation(&mut state);
        if state.reset {
    ...
    fn observe_transport_generation(&self, state: &mut ConnectedState) {
        if transport::current_transport_generation() != self.transport_generation {
            state.rx_payloads.clear();
            state.reset = true;
        }
    }
