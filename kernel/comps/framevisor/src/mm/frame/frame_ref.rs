...
use host_ostd::mm::{
    frame::FrameRef as OstdFrameRef,
    io::{
        VmIo,
...
impl<M: AnyUFrameMeta + ?Sized> FrameRef<'_, M> {
    ...
    /// Returns the number of live OSTD references to the borrowed frame.
    pub fn reference_count(&self) -> u64 {
        self.0.reference_count()
    }
}

impl<M: AnyUFrameMeta + ?Sized> HasVmReaderWriter for FrameRef<'_, M> {
