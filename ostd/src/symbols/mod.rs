...
pub fn symbols_table_init() {
    let sources = crate::boot::symbol_sources();
    if let Some(symbols) = sources.kernel_elf() {
...
fn parse_symbols_file_from_binary(symbols: crate::boot::KernelSymbolImage) {
    let symbols = symbols.bytes();
