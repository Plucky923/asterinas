...
//! Provides the FrameVM terminal subsystem.
//!
//! [`Tty`] coordinates job control, the line discipline, polling, and a [`TtyDriver`]; the
//! FrameV console driver connects that generic terminal interface to the host console frontend.
...
fn init_framev_console() -> Result<()> {
    let console =
        framev_pci::console().map_err(|err| Error::with_message(Errno::EINVAL, err.message()))?;
...
impl<D: TtyDriver> Tty<D> {
    ...
    pub fn push_input(&self, chs: &[u8]) -> Result<usize> {
        let mut echo_bytes = Vec::with_capacity(2);
