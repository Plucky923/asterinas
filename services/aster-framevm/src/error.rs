...
use aster_util::per_cpu_counter::PerCpuCounterError;
...
impl From<PerCpuCounterError> for Error {
    fn from(error: PerCpuCounterError) -> Self {
        let message = match error {
            PerCpuCounterError::NoCurrentFrameVcpu => {
                "per-CPU counter requires a current FrameVM vCPU"
            }
            PerCpuCounterError::FrameVmNotFound => {
                "the FrameVM owning the per-CPU counter is unavailable"
            }
            PerCpuCounterError::NoVcpus => "per-CPU counter requires at least one vCPU",
        };
        Error::with_message(Errno::EINVAL, message)
    }
}

impl From<ostd::FramevisorError> for Error {
    fn from(framevisor_error: ostd::FramevisorError) -> Self {
        match framevisor_error {
            ostd::FramevisorError::InvalidArgs => Error::new(Errno::EINVAL),
            ostd::FramevisorError::NoMemory => Error::new(Errno::ENOMEM),
            ostd::FramevisorError::PageFault => Error::new(Errno::EFAULT),
            ostd::FramevisorError::AccessDenied => Error::new(Errno::EPERM),
            ostd::FramevisorError::IoError => Error::new(Errno::EIO),
            ostd::FramevisorError::NotEnoughResources => Error::new(Errno::EBUSY),
            ostd::FramevisorError::Overflow => Error::new(Errno::EINVAL),
        }
    }
}
