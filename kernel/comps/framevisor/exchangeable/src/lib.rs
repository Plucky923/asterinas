...
pub enum DomainId {
    ...
    Host,
    /// Guest domain with numeric identifier.
    Guest(u32),
}
...
impl DomainId {
    ...
    /// Checks if this is a guest domain.
    #[inline]
    pub fn is_guest(&self) -> bool {
        matches!(self, DomainId::Guest(_))
    }

    /// Returns the guest ID if this is a guest domain.
    #[inline]
    pub fn guest_id(&self) -> Option<u32> {
        match self {
            DomainId::Guest(id) => Some(*id),
            DomainId::Host => None,
        }
    }
    ...
    ///
    /// Host is encoded as `0`; guest domain `n` is encoded as `n + 1`.
    #[inline]
    const fn to_u64(self) -> u64 {
        match self {
            DomainId::Host => 0,
            DomainId::Guest(id) => (id as u64) + 1,
        }
    }
...
// `RRef<T>` is the only approved way for one exchangeable object to refer to
// another exchange allocation without embedding ordinary Rust references or
// private heap pointers in the exchanged object graph.
...
mod tests {
    ...
    fn failed_transfer_returns_original_token() {
        ...
        let rref = RRef::new_with_owner(37u64, DomainId::Guest(1));
        ...
        assert_eq!(rref.owner(), DomainId::Guest(1));
        assert!(rref.try_get().is_none());

        drop(host_domain_guard);
        let _guest_domain_guard = enter_domain(DomainId::Guest(1));
    ...
    fn try_into_inner_rejects_non_owner_and_preserves_token() {
        ensure_test_registry();
        let host_domain_guard = enter_domain(DomainId::Host);
        let rref = RRef::new_with_owner(41u64, DomainId::Guest(2));
        ...
        assert_eq!(rref.owner(), DomainId::Guest(2));
        assert!(rref.try_get().is_none());

        drop(host_domain_guard);
        let _guest_domain_guard = enter_domain(DomainId::Guest(2));
        assert_eq!(
            rref.try_into_inner()
