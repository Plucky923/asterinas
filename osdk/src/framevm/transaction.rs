...
impl RunArtifactSource {
    const fn as_str(self) -> &'static str {
        match self {
            Self::NoBuildReuse => "no-build-reuse",
            Self::BootCarrierRefresh => "boot-carrier-refresh",
            Self::FrameVmOnlyRefresh => "framevm-only-refresh",
...
impl FrameVmTransaction {
    ...
    pub(super) fn run_artifact_set(
        ...
        let mut timings = report::StageTimings::new();
        let (bundle, runtime_config, artifact_source) = if no_build {
            timings.measure("no_build_artifact_set_validation", || {
                match self.try_load_existing_bundle_for_run(true, None)? {
                    Some((bundle, runtime_config)) => {
                        Ok((bundle, runtime_config, RunArtifactSource::NoBuildReuse))
                    }
            ...
            timings.measure("artifact_set_selection_or_build", || {
                if let Some(existing_artifact_set) = self.try_load_current_bundle_for_run()? {
                    let (bundle, runtime_config) = existing_artifact_set;
                    Ok((
                        bundle,
                        runtime_config,
                        RunArtifactSource::BootCarrierRefresh,
                    ))
                    ...
                    Ok((
                        artifact_set.bundle,
                        runtime_config,
                        RunArtifactSource::FrameVmOnlyRefresh,
                    ))
                } else {
                    ...
                    Ok((
                        artifact_set.bundle,
                        runtime_config,
                        RunArtifactSource::FullBuild,
                    ))
                }
            })?
        };

        let initramfs_path = bundle.initramfs_path().map_or_else(
            || "<missing>".to_string(),
            |path| path.display().to_string(),
        );
        let symbols_path = bundle.framevm_symbols_path().map_or_else(
            || "<missing>".to_string(),
            |path| path.display().to_string(),
        );
        eprintln!(
            "framevm runtime artifacts: source={}, bundle={}, object={}, symbols={}, initramfs={initramfs_path}",
            artifact_source.as_str(),
            self.bundle_path.display(),
            self.framevm_object_path().display(),
            symbols_path,
        );
        ...
        let case_name = self.config.load_action.case_name();
        if self.config.load_action.is_interactive() {
            ...
            timings.measure("qemu_launch_until_success", || {
                let exit_status = bundle
                    .run_qemu_until_log_match(
                        &runtime_config,
                        ActionChoice::Run,
                        self.config.load_action.success_timeout(),
                        |qemu_log| framevm_load_completed(qemu_log, case_name, success_markers),
                        |qemu_log| {
                            framevm_load_failure_summary(qemu_log, case_name, success_markers)
                        },
                    )
                    .map_err(FrameVmStageError::Run)?;
                if bundle::classify_qemu_exit_status(exit_status) == QemuExit::Success {
                    return Ok(());
                }

                let qemu_log = runtime_config.work_dir.join("qemu.log");
                let summary = fs::read_to_string(&qemu_log)
                    .map(|qemu_log| {
                        framevm_load_failure_summary(&qemu_log, case_name, success_markers)
                    })
                ...
                Err(FrameVmStageError::Run(format!(
                    "qemu exited with status {:?}; {summary}",
                    exit_status.code()
                )))
            })?;
        }
    ...
    fn build_validated_artifact_set(
        ...
        let host_symbol_target_dir = self.cargo_target_dir.join("framevm-host-symbols");
        let host_symbol_files = timings.measure("pre_host_symbol_rlib_selection", || {
            retention::find_policy_rlib_candidates(
                &host_symbol_target_dir,
                &staging_config.target,
                &staging_config.osdk_config.run.build.profile,
                &policy.host_symbols.rlibs,
            )
        })?;
        let supplemental_archives = timings.measure("pre_host_archive_rlib_selection", || {
            retention::find_policy_rlib_candidates(
                &host_symbol_target_dir,
                &staging_config.target,
                &staging_config.osdk_config.run.build.profile,
                &policy.host_symbols.archive_rlibs,
            )
        })?;
        ...
        if !host_validation.is_exact_match() {
            ...
            if refined_host_rustflags != host_retention_request {
    ...
    fn try_refresh_boot_carrier_for_current_artifacts(
        ...
        // A changed load script or inner memory limit changes the boot carrier,
        // not the FrameVM object. Treat an invalid artifact set as a normal
        // cache miss here, then let the full build path replace it. The
        // `--no-build` path validates the same condition strictly.
        if validate_existing_framevm_artifact_set(&bundle, &object_output).is_err() {
            return Ok(None);
        }
    ...
    fn try_load_current_build_artifact_set(
        ...
        // A build may encounter a bundle written by an older OSDK that does
        // not yet contain every artifact in the current set. This is a cache
        // miss, not an error: the normal build path below can replace it with
        // a complete, self-consistent bundle.
        if validate_existing_framevm_artifact_set(&bundle, &object_output).is_err() {
            return Ok(None);
        }
        ...
        let Some(symbols) = bundle.framevm_symbols_path() else {
            return Ok(None);
        };
        let Some(initramfs) = bundle.config().run.boot.initramfs.clone() else {
            return Ok(None);
        };
    ...
    fn runtime_config(&self, initramfs: &PathBuf) -> Config {
        ...
        replace_init_args(
            ...
            self.config
                .load_action
                .init_args_with_memory_limit(self.config.memory_limit.as_deref()),
        );
        config
    }
...
impl Drop for PublishedFileGuard {
    fn drop(&mut self) {
        ...
        if let Some(backup) = self.backup.take() {
            if fs::rename(&backup, &self.path).is_err() {
...
impl PublishedDirectoryGuard {
    fn replace(
        ...
        if let Err(error) = fs::rename(staging_path, published_path) {
            if error.kind() == std::io::ErrorKind::CrossesDevices {
                if let Err(error) = copy_dir_recursively(staging_path, published_path) {
                    let _ = remove_dir_if_exists(published_path);
...
fn framevm_load_completed(
    ...
    if let Some(case_name) = case_name {
        qemu_log
            .lines()
            .any(|line| line.trim() == format!("FRAMEVM_CASE_RESULT case={case_name} result=pass"))
    } else {
        success_markers
            .iter()
...
fn framevm_load_failure_summary(
    qemu_log: &str,
    case_name: Option<&str>,
    success_markers: &[&str],
) -> String {
    ...
    let missing_case_result = case_name
        .filter(|case_name| {
            !qemu_log.lines().any(|line| {
                line.trim() == format!("FRAMEVM_CASE_RESULT case={case_name} result=pass")
            })
        })
        .map(|case_name| format!("FRAMEVM_CASE_RESULT case={case_name} result=pass"))
        .unwrap_or_else(|| "none".to_string());
    let control_error = qemu_log
        .lines()
        .rev()
        .map(str::trim)
        .find(|line| line.starts_with("framevmm:") || line.contains("FRAMEVM_START"))
        .unwrap_or("no framevmm control error observed");

    format!(
        "missing framevm markers: {missing_markers}; missing case result: {missing_case_result}; {terminal_status}; {control_error}"
    )
}
...
mod tests {
    ...
    #[test]
    fn run_artifact_sources_have_stable_log_values() {
        assert_eq!(RunArtifactSource::NoBuildReuse.as_str(), "no-build-reuse");
        assert_eq!(
            RunArtifactSource::BootCarrierRefresh.as_str(),
            "boot-carrier-refresh"
        );
        assert_eq!(
            RunArtifactSource::FrameVmOnlyRefresh.as_str(),
            "framevm-only-refresh"
        );
        assert_eq!(RunArtifactSource::FullBuild.as_str(), "full-build");
    }

    #[test]
    fn framevm_run_accepts_encoded_qemu_success() {
        let encoded_success = std::process::ExitStatus::from_raw(33 << 8);

        assert_eq!(
            bundle::classify_qemu_exit_status(encoded_success),
            QemuExit::Success
        );
    }
    ...
    fn framevm_load_failure_summary_reports_missing_markers() {
        let summary = framevm_load_failure_summary(
            "FRAMEVM_BOOT_OK\nFrameVM terminal status: exited code=0\n",
            Some("boot"),
        ...
        assert_eq!(
            summary,
            "missing framevm markers: FRAMEVM_ROOTFS_OK; missing case result: FRAMEVM_CASE_RESULT case=boot result=pass; FrameVM terminal status: exited code=0; no framevmm control error observed"
        );
    }

    #[test]
    fn framevm_load_failure_summary_reports_missing_terminal_status() {
        let summary = framevm_load_failure_summary("", Some("load"), &["FRAMEVM_LOAD_OK"]);

        assert_eq!(
            summary,
            "missing framevm markers: FRAMEVM_LOAD_OK; missing case result: FRAMEVM_CASE_RESULT case=load result=pass; no framevm terminal status observed; no framevmm control error observed"
        );
    }

    #[test]
    fn framevm_load_failure_summary_reports_control_error() {
        let summary = framevm_load_failure_summary(
            "framevmm: start FrameVM draft: invalid argument\n",
            Some("load"),
            &["FRAMEVM_LOAD_OK"],
        );
        ...
        assert_eq!(
            summary,
            "missing framevm markers: FRAMEVM_LOAD_OK; missing case result: FRAMEVM_CASE_RESULT case=load result=pass; no framevm terminal status observed; framevmm: start FrameVM draft: invalid argument"
        );
    }

    #[test]
    fn framevm_load_completed_requires_the_exact_case_result() {
        assert!(!framevm_load_completed(
            "FRAMEVM_LOAD_OK\n",
            Some("load"),
            &["FRAMEVM_LOAD_OK"]
        ));
        assert!(!framevm_load_completed(
            "FRAMEVM_CASE_RESULT case=load result=fail class=guest-status\nFrameVM terminal status: exited code=0\n",
            Some("load"),
            &["FRAMEVM_LOAD_OK"]
        ));
        assert!(framevm_load_completed(
            "FRAMEVM_CASE_RESULT case=load result=pass\n",
            Some("load"),
            &["FRAMEVM_LOAD_OK"]
        ));
        assert!(framevm_load_completed(
            "FRAMEVM_CASE_RESULT case=load result=pass\nFrameVM terminal status: exited code=7\n",
            Some("load"),
            &["missing compatibility marker"]
        ));
        assert!(framevm_load_completed(
            "FRAMEVM_CASE_RESULT case=lifecycle result=pass\nFrameVM terminal status: exited code=1\n",
            Some("lifecycle"),
            &["FRAMEVM_LIFECYCLE_OK"]
        ));
        assert!(framevm_load_completed(
            "FRAMEVM_LOAD_OK\n",
            None,
