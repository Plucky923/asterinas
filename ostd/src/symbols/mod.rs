...
pub fn symbol_addr_by_name(name: &str) -> Option<usize> {
    ...
    // Try exact match first (for demangled names)
    ...
    // Try demangling the input name and match (ignoring hash suffixes)
    let demangled = demangle(name).to_string();
    let demangled_no_hash = strip_hash_suffix(&demangled);

    for (_addr, entry) in table.iter() {
        let entry_no_hash = strip_hash_suffix(&entry.name);
        if entry_no_hash == demangled_no_hash {
            return Some(entry.addr);
        }
    }

    // Also try matching the mangled name directly (in case it's stored)
    // This is less likely but worth trying
    for (_addr, entry) in table.iter() {
        // If the stored name is the same as input (mangled), return it
        if entry.name == name {
            return Some(entry.addr);
        }
    }
