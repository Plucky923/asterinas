...
use ostd::{
    arch::cpu::context::UserContext, mm::VmIo, sync::RwArc, task::Task, user::UserContextApi,
};
...
impl CloneFlags {
    fn check_unsupported_flags(&self) -> Result<()> {
        ...
        if !unsupported_flags.is_empty() {
            return_errno_with_message!(Errno::EINVAL, "contains unsupported clone flags");
        }
        Ok(())
    }
}
...
pub fn clone_child(
    ...
    if clone_args.flags.contains(CloneFlags::CLONE_THREAD) {
        ...
        // Race conditions are fine as we don't really care which CPU creates
        // a process. Process statistics are best effort and must not make a
        // successful clone panic when its FrameVM context is unavailable.
        if let (Some(counter), Some(vcpu_index)) = (
            PROCESS_CREATION_COUNTER.get(),
