...
use super::{ioctl_defs, vm_file::FrameVmFile};
use crate::{
    events::IoEvents,
    fs::{
...
impl FrameVmControllerFile {
    fn insert_vm_file(request: ioctl_defs::FrameVmCreateVm) -> Result<i32> {
        ...
        let fd = {
            ...
            let vm_file = FrameVmFile::new(
                request.vcpu_count() as usize,
                request.share(),
                usize::try_from(request.memory_limit_bytes()).map_err(|_| {
                    Error::with_message(Errno::EINVAL, "FrameVM memory limit overflows usize")
                })?,
                &current_task,
            );
    ...
    fn ioctl(&self, raw_ioctl: RawIoctl) -> Result<i32> {
        ...
        dispatch_ioctl!(match raw_ioctl {
            cmd @ CreateVm => {
                ...
                Self::insert_vm_file(request)
            }
            StartVm | StopVm | GetConsoleFd | GetStatus | GetMemoryStatus | SetCmdline
            | SetArtifact | AddConsole | AddRng | AddBlock | AddSock | AddNet | AssignPci => {
