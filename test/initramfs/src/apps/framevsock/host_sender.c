...
int main(int argc, char *argv[])
{
	if (argc < 4) {
		fprintf(stderr,
			"Usage: %s <guest_cid> <total_bytes> <buf_size> [port]\n",
			argv[0]);
	...
	/*
	 * FrameVsock host-side close() may reset active connections in socket
	 * drop path. Gracefully shutdown first so guest receiver can drain all
	 * queued data and observe EOF.
	 */
