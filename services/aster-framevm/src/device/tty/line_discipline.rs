...
impl LineDiscipline {
    ...
    pub fn push_char<F: FnMut(SigNum)>(
        ...
        let signal = char_to_signal(ch, &self.termios);
    ...
    /// Returns whether a blocking read can make progress under the current termios settings.
...
fn char_to_signal(ch: u8, termios: &CTermios) -> Option<SigNum> {
    ...
    if matches_special_char(ch, termios, CCtrlCharId::VINTR) {
        Some(SIGINT)
    } else if matches_special_char(ch, termios, CCtrlCharId::VQUIT) {
        Some(SIGQUIT)
    } else {
        None
    }
