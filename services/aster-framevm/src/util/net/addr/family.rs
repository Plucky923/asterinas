...
use super::{unix, vsock::CSocketAddrVm};
...
pub fn read_socket_addr_from_user(addr: Vaddr, addr_len: usize) -> Result<SocketAddr> {
    ...
    let result = match CSocketAddrFamily::try_from(storage.sa_family as i32) {
        Ok(CSocketAddrFamily::AF_INET) => {
            return_errno_with_message!(Errno::EAFNOSUPPORT, "AF_INET is not supported")
        }
        Ok(CSocketAddrFamily::AF_INET6) => {
            return_errno_with_message!(Errno::EAFNOSUPPORT, "AF_INET6 is not supported")
        }
        ...
        Ok(CSocketAddrFamily::AF_NETLINK) => {
            return_errno_with_message!(Errno::EAFNOSUPPORT, "AF_NETLINK is not supported")
        }
