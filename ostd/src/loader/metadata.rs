...
fn find_framevm_metadata_section<'a>(elf_file: &'a ElfFile) -> Result<SectionHeader<'a>> {
    ...
    for (section_index, section) in elf_file.section_iter().enumerate() {
        ...
        if metadata_section.replace(section).is_some() {
            return Err(invalid_args(
                "service module contains multiple `.framevm.meta` sections",
            ));
        }
    }
    metadata_section.ok_or_else(|| invalid_args("service module object is missing `.framevm.meta`"))
}
