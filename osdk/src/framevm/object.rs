...
struct BuildReport<'a> {
    ...
    on_demand_rlibs: &'a [retention::OnDemandRlib],
...
fn link_final_object(
    ...
    let on_demand_rlibs = retention::find_on_demand_rlibs(
        policy,
        service_target_dir,
        target,
        &build.profile,
    ...
    let on_demand_archives = on_demand_rlibs
        .iter()
        .map(|rlib| rlib.rlib.clone())
        .collect::<Vec<_>>();
    retention::run_ld_relocatable_with_archives(
        output_object,
...
fn find_or_build_host_symbol_rlibs(
    ...
    let host_symbol_target_dir = host_symbol_target_dir(host_target_dir);
    build_host_symbol_rlibs(workspace_root, &host_symbol_target_dir, target, build)?;
...
fn host_symbol_target_dir(host_target_dir: &Path) -> PathBuf {
    host_target_dir.join(HOST_SYMBOL_TARGET_DIR_NAME)
}
...
fn build_host_symbol_rlibs(
    ...
    let host_rustflags = host_rustflags.join(" ");
    let mut command = new_host_symbol_command(&host_rustflags);
    command.arg("build");
    configure_host_symbol_command(
        &mut command,
        &kernel_crate.name,
        host_target_dir,
        target,
    ...
    command.args(["--", "-C", "link-dead-code"]);

    process::run_status(
        command,
        "retaining framevm host service ABI symbols",
        FrameVmStageError::ObjectBuild,
    )
}

fn new_host_symbol_command(host_rustflags: &str) -> Command {
    let mut command = process::command("cargo");
    command.env("RUSTFLAGS", host_rustflags);
    command.env("CFLAGS_x86_64-unknown-none", FRAMEVM_CFLAGS);
    command.env("CARGO_INCREMENTAL", "0");
    command
}

fn configure_host_symbol_command(
    command: &mut Command,
    package: &str,
    host_target_dir: &Path,
    target: &str,
    ...
    imports
        ...
        .map(|symbol| {
            ...
            if class == ImportClass::AllowedHostDynamic {
                if let Some(host_match) = host_index
                    .raw
                    .get(&symbol.raw)
                    .and_then(|matches| matches.first())
                {
                    // Multiple rlibs may contain the same exact raw symbol.
                    // They are duplicate locations for one ABI identity, not
                    // ambiguous normalized-symbol candidates.
                    host_matches.insert(host_match.clone());
                }
                if host_matches.is_empty()
                    && let Some(matches) = host_index.demangled.get(&symbol.demangled)
                    && let Some(host_match) = matches.first()
                {
                    // Candidate rlibs can contain several Cargo identities for
                    // the same normalized API. Final-host validation resolves
                    // the identity that is actually present in the Host ELF.
                    host_matches.insert(host_match.clone());
                }
            }
...
fn write_reports(
    ...
    on_demand_rlibs: &[retention::OnDemandRlib],
...
mod tests {
    ...
    #[test]
    fn host_symbol_target_dir_does_not_reuse_the_regular_kernel_cache() {
        let host_target_dir = Path::new("/tmp/kernel-target");

        assert_eq!(
            host_symbol_target_dir(host_target_dir),
            PathBuf::from("/tmp/kernel-target/framevm-host-symbols")
        );
    }
}
