...
fn do_inotify_init(flags: u32, ctx: &Context) -> Result<SyscallReturn> {
    ...
    let fd = file_table.write().insert(file, FdFlags::empty());
...
pub fn sys_inotify_rm_watch(fd: FileDesc, wd: i32, ctx: &Context) -> Result<SyscallReturn> {
    ...
    println!("inotify_rm_watch wd = {}", wd);
