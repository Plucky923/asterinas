...
/// Describes whether a receive operation consumes or peeks data.
...
impl ReceiveBehavior {
    pub const fn will_consume_data(&self) -> bool {
...
impl SendRecvFlags {
    ...
    pub fn ensure_supported_by(&self, supported_flags: Self) -> Result<()> {
        if supported_flags.contains(*self) {
            return Ok(());
        }

        return_errno_with_message!(Errno::EOPNOTSUPP, "unsupported send or receive flag");
    }
