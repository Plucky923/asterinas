...
pub fn symbols_table_init() {
    ...
    framevm::init_from_boot_payload(crate::boot::boot_info().framevm_symbols);
}
...
/// A diagnostic entry from the FrameVM runtime symbol table.
#[derive(Clone, Copy, Debug)]
pub struct FrameVmSymbolEntry<'a> {
    /// Complete raw symbol name used by exact FrameVM import lookup.
    pub raw_name: &'a [u8],
    /// Resolved host address recorded in `framevm.symbols`.
    pub addr: usize,
}
...
type KernelSymbolTable = SpinLock<SymbolTableInner>;
type CrateAliases = SpinLock<BTreeMap<Arc<str>, Vec<Arc<str>>>>;

static SYMBOLS_TABLE: Once<KernelSymbolTable> = Once::new();
static CRATE_ALIASES: Once<CrateAliases> = Once::new();

fn symbol_table() -> &'static KernelSymbolTable {
...
/// Returns whether FrameVM exact symbol lookup is available.
pub fn framevm_symbol_table_available() -> bool {
    framevm::is_available()
}

/// Looks up a FrameVM host import by complete raw symbol-name bytes.
pub fn framevm_symbol_addr_by_name(raw_name: &[u8]) -> Option<usize> {
    framevm::lookup(raw_name)
}

/// Returns the SHA-256 hash of the boot-provided FrameVM symbol table.
pub fn framevm_symbols_payload_hash() -> Option<[u8; 32]> {
    framevm::payload_hash()
}

/// Retrieves a FrameVM symbol-table diagnostic entry by exact address.
pub fn framevm_symbol_by_addr(addr: usize) -> Option<FrameVmSymbolEntry<'static>> {
    framevm::lookup_by_addr(addr).map(|symbol| FrameVmSymbolEntry {
        raw_name: symbol.raw_name(),
        addr: symbol.addr(),
    })
}

/// Iterates over FrameVM symbol-table diagnostic entries.
pub fn for_each_framevm_symbol(mut callback: impl FnMut(FrameVmSymbolEntry<'static>) -> bool) {
    framevm::for_each_symbol(|symbol| {
        callback(FrameVmSymbolEntry {
            raw_name: symbol.raw_name(),
            addr: symbol.addr(),
        })
    });
