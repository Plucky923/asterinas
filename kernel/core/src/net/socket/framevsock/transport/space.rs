...
use framev_sock_common::{ConnectionId, FrameVsockPacket, VsockOp};
...
use crate::{
    net::socket::framevsock::{
        addr::{FrameVsockAddr, VMADDR_CID_ANY, VMADDR_CID_HOST},
...
impl FrameVsockSpace {
    ...
    fn handle_request(
        ...
        let listen = self.get_listen_socket(&dst_addr).or_else(|| {
            self.get_listen_socket(&FrameVsockAddr::new(VMADDR_CID_ANY, dst_addr.port))
        });
