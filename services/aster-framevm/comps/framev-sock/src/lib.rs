...
pub fn activate() -> Result<(), FrameVPciError> {
    framev_pci::sock()?.activate()
}

/// Installs the RX notification callback for the default `framev-sock` device.
pub fn install_rx_callback(callback: fn()) -> Result<(), FrameVPciError> {
    framev_pci::sock()?.install_rx_callback(callback)
}

/// Returns whether the default `framev-sock` transport is active.
pub fn is_active() -> bool {
    framev_pci::sock().is_ok_and(|sock| sock.is_active())
}
...
pub fn guest_cid() -> Option<u64> {
    framev_pci::sock().ok().and_then(|sock| sock.guest_cid())
}
...
pub fn submit_packet(
    ...
    let Ok(sock) = framev_pci::sock() else {
        return Err((FrameVsockSendError::Stopped, packet));
    };
...
pub fn take_transport_reset() -> Option<u64> {
    framev_pci::sock()
        .ok()
        .and_then(|sock| sock.take_transport_reset())
}
