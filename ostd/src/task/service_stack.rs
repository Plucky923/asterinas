...
cpu_local! {
    ...
    static CPU_SERVICE_STACK_PTR: AtomicUsize = AtomicUsize::new(0);
}
...
pub fn init(cpu_count: usize) {
    SERVICE_STACKS.call_once(|| {
        ...
        for _ in 0..cpu_count {
            stacks.push(ServiceStack::new().expect("Failed to allocate service stack"));
        }
        stacks
    });
}
...
pub fn init_on_cpu(cpu_id: usize) {
    let Ok(cpu_id) = CpuId::try_from(cpu_id) else {
        return;
    };

    if let Some(stacks) = SERVICE_STACKS.get() {
        if let Some(stack) = stacks.get(cpu_id.as_usize()) {
            CPU_SERVICE_STACK_PTR
                .get_on_cpu(cpu_id)
                .store(stack as *const _ as usize, Ordering::Release);
        }
    }
}
...
fn get_current_service_stack() -> Option<&'static ServiceStack> {
    let current_cpu = crate::irq::disable_local().current_cpu();
    let ptr = CPU_SERVICE_STACK_PTR
    ...
    F: FnOnce() -> R,
{
    ...
    let service_stack =
        get_current_service_stack().expect("Service stack not initialized for this CPU");
...
/// Executes a closure directly on architectures without service-stack switching.
