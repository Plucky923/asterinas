...
/// Set to `true` to skip all Registry operations and ID generation.
/// This helps identify if RRef overhead is the throughput bottleneck.
/// WARNING: Do NOT enable in production - breaks crash recovery!
const SKIP_REGISTRY_FOR_PERF_TEST: bool = false;
...
fn generate_rref_id() -> RRefId {
    if SKIP_REGISTRY_FOR_PERF_TEST {
        // Skip atomic operation - return dummy ID
        return 1;
    }
...
impl<T: Exchangeable> RRef<T> {
    ...
    pub fn new(value: T) -> Self {
        ...
        // Skip registry for performance testing
        if !SKIP_REGISTRY_FOR_PERF_TEST {
