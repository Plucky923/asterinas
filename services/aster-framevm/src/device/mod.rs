...
//! Device providers for the FrameVM kernel image.
...
pub(crate) use misc::{getrandom, getrandom_bytes, geturandom};
...
/// Initializes device providers used by copied kernel code.
pub fn init_in_first_kthread() -> crate::error::Result<()> {
    tty::init()?;
    misc::init()
}

/// Initializes devices in the first process.
