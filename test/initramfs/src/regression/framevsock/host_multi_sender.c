 ...
 *
 * FrameVsock Multi-Connection Throughput Benchmark (Asterinas Host Sender)
 *
 * This program runs on Asterinas Host and opens multiple concurrent
 * connections to a FrameVM Guest, sending a fixed number of bytes per
 * connection. One thread per connection.
 *
 * Usage:
 *   ./host_multi_sender <num_threads> <guest_cid> <buf_size> <bytes_per_conn> [port] [warmup_bytes]
 *
 * Example:
 *   ./host_multi_sender 4 3 65536 1073741824
 */
...
struct sender_args {
	...
	uint64_t send_calls;
	uint64_t eagain_retries;
	int error;
};

static inline uint64_t load_u64(const uint64_t *p)
{
	return __atomic_load_n(p, __ATOMIC_RELAXED);
}

static inline void store_u64(uint64_t *p, uint64_t v)
{
	__atomic_store_n(p, v, __ATOMIC_RELAXED);
}

static inline int load_i32(const int *p)
{
	return __atomic_load_n(p, __ATOMIC_RELAXED);
}

static inline void store_i32(int *p, int v)
{
	__atomic_store_n(p, v, __ATOMIC_RELAXED);
}
...
static void *sender_thread(void *arg)
{
	...
	while (args->bytes_sent < args->bytes_per_conn) {
		...
		if (ret < 0 && (errno == EINTR || errno == EAGAIN ||
				errno == EWOULDBLOCK)) {
			eagain_count++;
			store_u64(&args->eagain_retries, eagain_count);
			continue;
		}
		if (ret <= 0) {
			store_i32(&args->error, errno ? errno : EIO);
			break;
		}
		args->bytes_sent += (uint64_t)ret;
		send_calls++;
		store_u64(&args->send_calls, send_calls);
		store_u64(&args->bytes_sent, args->bytes_sent);
	}
	send_done = (load_i32(&args->error) == 0 &&
		     args->bytes_sent == args->bytes_per_conn);

	clock_gettime(CLOCK_MONOTONIC, &end);
	store_u64(&args->duration_ns,
...
out:
	...
	if (fd >= 0) {
		/*
		 * FrameVsock host-side close() may reset active connections in
		 * socket drop path. Gracefully shutdown first so guest receiver
		 * can drain all queued data and observe EOF.
		 */
		if (send_done) {
...
int main(int argc, char *argv[])
{
	...
	uint64_t guest_cid = strtoull(argv[2], NULL, 10);
	...
	printf("\n========================================\n");
	printf(" FrameVsock Multi-Connection Sender\n");
	printf("========================================\n");
	printf(" Threads:    %d\n", num_threads);
	printf(" Target:     CID %llu, Port %u\n",
	       (unsigned long long)guest_cid, port);
	...
	for (int i = 0; i < num_threads; i++) {
		...
		store_u64(&args[i].bytes_sent, 0);
		store_u64(&args[i].duration_ns, 0);
		store_u64(&args[i].send_calls, 0);
		store_u64(&args[i].eagain_retries, 0);
		store_i32(&args[i].error, 0);
