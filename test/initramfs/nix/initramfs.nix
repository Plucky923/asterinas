...
let
  ...
  all_pkgs = [ busybox etc resolv_conf framevmm ]
...
in stdenvNoCC.mkDerivation {
  ...
  buildCommand = ''
    ...
    ${lib.optionalString (framevmRootfs != null) ''
      mkdir -p $out/framevm
      ${pkgs.gzip}/bin/gzip -n -c ${framevmRootfs} > $out/framevm/rootfs.ext2.gz
      stat -c %s ${framevmRootfs} > $out/framevm/rootfs.ext2.size
    ''}
