...
/// Unpack and prepare the rootfs from the initramfs CPIO buffer.
pub fn init_container_rootfs(initramfs_buf: &[u8]) -> Result<Arc<MountNode>> {
    ...
    let reader = {
        ...
        println!(
            "[Container] unpacking the initramfs.cpio{} to rootfs ...",
            initramfs_suffix
        );
    ...
    let mut decoder = CpioDecoder::new(reader);
    let fs = FsResolver::new_container(root_mount.clone());
    ...
    println!("[Container] rootfs is ready");
