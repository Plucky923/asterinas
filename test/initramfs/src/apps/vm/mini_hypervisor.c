...
static void print_stats(const char *name, int count)
{
	...
	printf("  %-28s min=%5lu  avg=%5lu  p50=%5lu  p99=%5lu cycles\n", name,
	       min, total / count, samples[count / 2],
...
static void bench_cpuid(void)
{
	...
	volatile uint32_t *ts_before_lo =
		(volatile uint32_t *)((uint8_t *)mem + TS_BEFORE);
	volatile uint32_t *ts_before_hi =
		(volatile uint32_t *)((uint8_t *)mem + TS_BEFORE + 4);
	volatile uint32_t *ts_after_lo =
		(volatile uint32_t *)((uint8_t *)mem + TS_AFTER);
	volatile uint32_t *ts_after_hi =
		(volatile uint32_t *)((uint8_t *)mem + TS_AFTER + 4);
