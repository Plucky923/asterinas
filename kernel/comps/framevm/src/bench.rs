...
use aster_framevisor::{DomainId, RRef, bench as fv_bench, vsock::set_guest_active};
...
use fv_bench::{rdtsc_end, rdtsc_start};
...
pub fn bench_rref_transfer() -> BenchStats {
    ...
    let mut rrefs: Vec<RRef<u64>> = (0..BENCH_ITERATIONS).map(|i| RRef::new(i as u64)).collect();
...
pub fn bench_rref_drop() -> BenchStats {
    ...
    // Warmup
    for i in 0..WARMUP_ITERATIONS {
        let Some(rref) = rrefs[i].take() else {
            break;
        };
        let start = rdtsc_start();
        drop(rref);
    ...
    // Measurement
    for i in 0..BENCH_ITERATIONS {
        let Some(rref) = rrefs[WARMUP_ITERATIONS + i].take() else {
            break;
        };
        let start = rdtsc_start();
        drop(rref);
...
pub fn run_all_benchmarks() {
    ...
    framevm_logln!(" FrameVisor Microbenchmark Suite");
    framevm_logln!(
        "================================================================================"
    );
    framevm_logln!(" Warmup iterations:      {}", WARMUP_ITERATIONS);
    framevm_logln!(" Measurement iterations: {}", BENCH_ITERATIONS);
    framevm_logln!(
        "--------------------------------------------------------------------------------\n"
    );

    // Baseline measurements
    framevm_logln!(" Baseline Measurements:");
    framevm_logln!(" ----------------------");
    ...
    framevm_logln!(" Summary (overhead in cycles, lower is better)");
    framevm_logln!(
        "================================================================================"
    );
    framevm_logln!(
    ...
    framevm_logln!(
        ...
        empty_call_stats.min,
        empty_call_stats.avg,
        empty_call_stats.p50
    );
    framevm_logln!(
    ...
    framevm_logln!(
        ...
        rref_create_stats.min,
        rref_create_stats.avg,
        rref_create_stats.p50
    );
    framevm_logln!(
        " RRef transfer:        min={:<4} avg={:<4} p50={:<4} cycles",
        rref_transfer_stats.min,
        rref_transfer_stats.avg,
        rref_transfer_stats.p50
    );
    framevm_logln!(
    ...
    framevm_logln!(
        ...
        force_switch_stats.min,
        force_switch_stats.avg,
        force_switch_stats.p50
    );
    framevm_logln!(
        "================================================================================\n"
    );
