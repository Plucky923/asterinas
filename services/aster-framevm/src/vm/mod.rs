...
use ostd::{
    mm::{
        CachePolicy, FrameAllocOptions, MAX_USERSPACE_VADDR, PageFlags, PageProperty, VmIo,
        VmQueriedItem, VmSpace, frame::segment::Segment, tlb::TlbFlushOp,
    },
    sync::Once,
...
/// Returns the total physical memory reported by boot information.
...
pub fn protect_range(
    ...
    while cursor.virt_addr() < range.end {
        let remaining_len = range.end - cursor.virt_addr();
        if cursor
            .protect_next(remaining_len, |flags, cache_policy| {
                *flags = page_flags;
    ...
    cursor
        .flusher()
        .issue_tlb_flush(TlbFlushOp::for_range(range));
...
fn map_zeroed_anonymous_with_cache(
    ...
    for (page_idx, frame) in segment.into_iter().enumerate() {
        ...
        let is_mapped = cursor.query().map_err(Error::from)?.1.is_some();
        cursor.jump(page_start).map_err(Error::from)?;
        if is_mapped {
