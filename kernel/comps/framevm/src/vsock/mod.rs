...
pub fn register_port(port: u32, socket: Arc<FrameVsockSocket>) -> Result<()> {
    let mut index = port_index().write();
    if index.contains_key(&port) {
        return Err(Error::with_message(
            Errno::EADDRINUSE,
            "FrameVsock port is already registered",
        ));
    }
...
pub fn register_connection(
    ...
    if index.contains_key(&id) {
        return Err(Error::with_message(
            Errno::EADDRINUSE,
            "FrameVsock connection is already registered",
        ));
    }
    index.insert(id, socket);
    Ok(())
}
