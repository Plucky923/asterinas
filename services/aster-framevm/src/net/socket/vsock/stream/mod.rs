...
///
/// The FrameV IRQ callback must only reach this scheduler hook. It must not
...
fn process_rx_notification() {
    if let Some(generation) = framev_sock_frontend::take_transport_reset() {
        super::transport::handle_transport_reset(generation);
    }
