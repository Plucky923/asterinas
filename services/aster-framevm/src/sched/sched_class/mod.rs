...
pub struct ClassScheduler {
    ...
    ///
    /// Inner runqueues are also touched by virtual interrupt callbacks, so
    /// their lock must suppress local virtual IRQ delivery independently of
    /// the Host runqueue lock.
