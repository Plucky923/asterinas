...
use ostd::{cpu::CpuId, sync::SpinLock, task::disable_preempt, timer::Jiffies};
use paste::paste;
use spin::Once;
...
type TimerManagerSlots = Vec<Once<Arc<TimerManager>>>;

fn init_timer_manager_slots(slots: &'static Once<TimerManagerSlots>) {
    slots.call_once(|| (0..ostd::cpu::num_cpus()).map(|_| Once::new()).collect());
}

fn timer_manager_on_cpu(
    slots: &'static Once<TimerManagerSlots>,
    cpu: CpuId,
) -> &'static Once<Arc<TimerManager>> {
    &slots.get().unwrap()[cpu.as_usize()]
}

fn current_timer_manager(slots: &'static Once<TimerManagerSlots>) -> &'static Arc<TimerManager> {
    let preempt_guard = disable_preempt();
    timer_manager_on_cpu(slots, preempt_guard.current_cpu())
        .get()
        .unwrap()
}
...
impl RealTimeClock {
    ...
    pub fn timer_manager() -> &'static Arc<TimerManager> {
        current_timer_manager(&CLOCK_REALTIME_MANAGER)
    }
}
...
impl MonotonicClock {
    ...
    /// Get the cpu-local system-wide `TimerManager` singleton of this clock.
    pub fn timer_manager() -> &'static Arc<TimerManager> {
        current_timer_manager(&CLOCK_MONOTONIC_MANAGER)
    }
}
...
impl BootTimeClock {
    ...
    /// Get the cpu-local system-wide `TimerManager` singleton of this clock.
    pub fn timer_manager() -> &'static Arc<TimerManager> {
        current_timer_manager(&CLOCK_BOOTTIME_MANAGER)
    }
}
...
macro_rules! define_timer_managers {
    ($($clock_id:ident,)*) => {
        $(
            paste! {
                pub static [<$clock_id _MANAGER>]: Once<TimerManagerSlots> = Once::new();
            }
        )*
        ...
        fn _init_system_wide_timer_managers() {
            $(
                ...
                paste! {
                    init_timer_manager_slots(&[<$clock_id _MANAGER>]);
                }
                for cpu in ostd::cpu::all_cpus() {
                    paste! {
                        timer_manager_on_cpu(&[<$clock_id _MANAGER>], cpu).call_once(|| timer_manager.clone());
                    }
                }
                let callback = || {
                    ...
                    paste! {
                        timer_manager_on_cpu(&[<$clock_id _MANAGER>], cpu).get().unwrap().process_expired_timers();
                    }
                };
                aster_softirq::timer::register_callback(callback);
            )*
        }
    }
}
...
fn init_coarse_clock() {
    ...
    aster_softirq::timer::register_callback(update_coarse_clock);
}
...
pub fn init_for_ktest() {
    ...
    init_timer_manager_slots(&CLOCK_REALTIME_MANAGER);
    for cpu in ostd::cpu::all_cpus() {
        timer_manager_on_cpu(&CLOCK_REALTIME_MANAGER, cpu).call_once(|| {
