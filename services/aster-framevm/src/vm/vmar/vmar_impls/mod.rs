...
impl Vmar {
    ...
    pub(super) fn new(process_vm: ProcessVm) -> Result<Arc<Self>> {
        ...
        let rss_counters = array::try_from_fn(|_| PerCpuCounter::new())?;
        Ok(Arc::new(Vmar {
    ...
    fn add_rss_counter(&self, rss_type: RssType, val: isize) -> bool {
        // There are races but updating a remote counter won't cause any
        // problems. RSS accounting is advisory and must not panic while a
        // task is leaving its FrameVM context.
        let Some(vcpu_index) = ostd::task::current_cpu_index() else {
            return false;
        };
        self.rss_counters[rss_type as usize]
            .add_on_cpu(vcpu_index, val)
            .is_some()
    }
}
...
impl Drop for RssDelta<'_> {
    fn drop(&mut self) {
        for i in 0..NUM_RSS_COUNTERS {
            ...
            let _ = self.operated_vmar.add_rss_counter(rss_type, delta);
        }
    }
}
