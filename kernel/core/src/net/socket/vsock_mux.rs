...
impl VsockMuxStreamSocket {
    ...
    fn connected_provider(&self) -> Result<Provider> {
        ...
        match &*state {
            ...
            State::Init(_) | State::Transitioning | State::Connecting { .. } | State::Listen(_) => {
                return_errno_with_message!(Errno::ENOTCONN, "the socket is not connected")
            }
        }
    }

    fn choose_connect_provider(pair: &ProviderPair, vsock_addr: VsockSocketAddr) -> Provider {
        if crate::net::socket::framevsock::is_framevm_cid(vsock_addr.cid.into()) {
            Provider::FrameV(pair.framev.clone())
        } else {
            Provider::Virtio(pair.virtio.clone())
        }
    }
}
...
impl ProviderPair {
    ...
    fn addr(&self) -> Result<SocketAddr> {
        if self.virtio_bound || (!self.framev_bound && !self.framev_listening) {
            self.virtio.addr()
        } else {
            self.framev.addr()
        }
...
impl Pollable for VsockMuxStreamSocket {
    fn poll(&self, mask: IoEvents, poller: Option<&mut PollHandle>) -> IoEvents {
        ...
        match &*state {
            ...
            State::Transitioning => IoEvents::empty(),
            State::Connecting { provider, .. } => provider.poll(mask, poller),
...
impl Socket for VsockMuxStreamSocket {
    ...
    fn connect(&self, socket_addr: SocketAddr) -> Result<()> {
        ...
        let provider = {
            ...
            let State::Init(pair) = &*state else {
                return_errno_with_message!(
                    Errno::EINVAL,
                    "the socket cannot start a new connection"
                );
            };
            let provider = Self::choose_connect_provider(pair, vsock_addr);
            let State::Init(pair) = core::mem::replace(&mut *state, State::Transitioning) else {
                unreachable!();
            };
        ...
        let connect_result = provider.connect(SocketAddr::Vsock(vsock_addr));
        let mut state = self.state.write();
        let State::Connecting { pair, .. } = core::mem::replace(&mut *state, State::Transitioning)
        else {
            unreachable!();
        };
        match connect_result {
            Ok(()) => {
                *state = State::Connected(provider);
                Ok(())
            }
            Err(error) => {
                *state = State::Init(pair);
                Err(error)
            }
        }
    }
    ...
    fn addr(&self) -> Result<SocketAddr> {
        let state = self.state.read();
        match &*state {
            State::Init(pair) | State::Listen(pair) => pair.addr(),
            State::Transitioning => unreachable!(),
            State::Connecting { provider, .. } => provider.addr(),
    ...
    fn get_option(&self, option: &mut dyn SocketOption) -> Result<()> {
        ...
        match &*state {
            ...
            State::Transitioning => unreachable!(),
            State::Connecting { pair, .. } => pair.get_option(option),
    ...
    fn set_option(&self, option: &dyn SocketOption) -> Result<()> {
        ...
        match &*state {
            ...
            State::Transitioning => unreachable!(),
            State::Connecting { pair, .. } => pair.set_option(option),
