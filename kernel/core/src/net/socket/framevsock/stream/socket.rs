...
#[derive(Default)]
struct PendingSend {
    packet: Option<RRef<FrameVsockPacket>>,
    reported_to_caller: bool,
}
...
impl FrameVsockStreamSocket {
    ...
    fn connected_for_io(&self) -> Result<Arc<Connected>> {
        let finished_attempt = match &*self.status.read() {
            Status::Connecting(attempt) if attempt.connecting.has_result() => Some(attempt.clone()),
            _ => None,
        };
        ...
        match &*self.status.read() {
            Status::Connected(connected) => Ok(connected.clone()),
    ...
    fn send_connect_request(
        ...
        let local_addr = attempt.connecting.local_addr();
        let request_packet =
            FrameVsockPacket::request(local_addr, remote_addr, DEFAULT_BUF_ALLOC, 0);

        if backend::send_packet(0, backend::PacketCarrier::from_owned(request_packet)).is_err() {
    ...
    fn send(&self, reader: &mut dyn MultiRead, flags: SendRecvFlags) -> Result<usize> {
        ...
        if pending_send.packet.is_some() {
            match connected.try_send(reader, flags, &mut pending_send.packet) {
                Ok(sent) if !pending_send.reported_to_caller => return Ok(sent),
                Ok(_) => pending_send.reported_to_caller = false,
                Err(error) => return Err(error),
            }
        }

        if self.is_nonblocking() {
            let before = reader.sum_lens();
            let result = connected.try_send(reader, flags, &mut pending_send.packet);
            if result
                .as_ref()
                .is_err_and(|error| error.error() == Errno::EAGAIN)
                && pending_send.packet.is_some()
            {
                let consumed = before.saturating_sub(reader.sum_lens());
                if consumed != 0 {
                    pending_send.reported_to_caller = true;
                    return Ok(consumed);
                }
            }
            return result;
...
impl Socket for FrameVsockStreamSocket {
    ...
    fn shutdown(&self, cmd: SockShutdownCmd) -> Result<()> {
        self.connected_for_io()?.shutdown(cmd)
    }
    ...
    fn drop(&mut self) {
        ...
        match inner {
            ...
            Status::Connecting(attempt) => {
                let local_addr = attempt.connecting.local_addr();
                let _ = vsockspace.remove_connecting_socket(&local_addr);
                if attempt.connecting.is_connected() {
                    let _ = vsockspace.recycle_port(&local_addr.port);
                }
            }
            Status::Listen(listen) => {
