...
impl AtomicAddr {
    ...
    #[inline]
    fn clear_if(&self, addr: FrameVsockAddr) -> bool {
        let mut current = self.inner.lock();
        if current.is_some_and(|current| current == addr) {
            *current = None;
            true
        } else {
            false
...
pub struct FrameVsockSocket {
    ...
    accepted: AtomicBool,
    /// Whether this socket owns and should unregister its local port index entry.
...
impl FrameVsockSocket {
    ...
    fn unregister_local_port_if_owned(&self, local: FrameVsockAddr) {
        if self.owns_local_port {
            super::unregister_port(local.port);
        }
    }

    fn unregister_connection_indexes(&self) {
        if let (Some(local), Some(peer)) = (self.local_addr.load(), self.peer_addr.load()) {
            super::unregister_connection(local.port, peer.cid, peer.port);
            self.unregister_local_port_if_owned(local);
        }
    }
    ...
    fn restore_connect_state_after_failure(&self, local_addr: FrameVsockAddr, auto_bound: bool) {
        if auto_bound {
            self.local_addr.clear_if(local_addr);
            super::unregister_port(local_addr.port);
            self.state.store(SocketState::Init);
        } else {
            self.state.store(SocketState::Bound);
        }
    }
    ...
    pub fn bind(self: &Arc<Self>, addr: FrameVsockAddr) -> Result<()> {
        ...
        if let Err(e) = super::register_port(local.port, self.clone()) {
            self.state.store(SocketState::Init);
            return Err(e);
        }
    ...
    pub fn listen(&self, backlog: u32) -> Result<()> {
        match self.state.load() {
            ...
            _ => return_errno_with_message!(Errno::EINVAL, "socket not bound"),
        }
    ...
    pub fn connect(self: &Arc<Self>, peer: FrameVsockAddr) -> Result<()> {
        ...
        let auto_bound = match state {
            SocketState::Init => {
                // Auto-bind
                self.bind(FrameVsockAddr::any())?;
                true
            }
            SocketState::Bound => false,
        ...
        {
            self.restore_connect_state_after_failure(local_addr, auto_bound);
            return Err(e);
        }
    ...
    pub fn close(&self) -> Result<()> {
        ...
        if has_connection {
            ...
            self.unregister_connection_indexes();
        } else {
    ...
    fn reset_connected_socket(&self) {
        if let (Some(_local), Some(peer)) = (self.local_addr.load(), self.peer_addr.load()) {
