...
pub fn sys_capget(cap_user_header_addr: Vaddr, cap_user_data_addr: Vaddr) -> Result<SyscallReturn> {
    ...
    let inheritablecap = credentials.inheritablecap();
    let permittedcap = credentials.permittedcap();
    let effectivecap = credentials.effectivecap();
    ...
    let result = cap_user_data_t {
        ...
        effective: effectivecap.as_u32(),
        permitted: permittedcap.as_u32(),
        inheritable: inheritablecap.as_u32(),
    };
