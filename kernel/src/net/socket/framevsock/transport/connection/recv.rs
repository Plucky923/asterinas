...
impl PendingRead {
    fn remaining_data(&self) -> &[u8] {
        match self {
            Self::Partial(partial) => &partial.packet.payload()[partial.offset..],
            Self::Packet(packet) => packet.payload(),
        }
    }

    fn total_packet_len(&self) -> usize {
        match self {
            Self::Partial(partial) => partial.packet.payload().len(),
            Self::Packet(packet) => packet.payload().len(),
        }
    }
...
impl Connected {
    ...
    ///
    /// Zero-copy path:
    /// 1. Get packet from pending queue (RRef<FrameVsockPacket>)
    ...
    /// stalling forever.
    pub fn on_packet_received(&self, packet: RRef<FrameVsockPacket>) -> Result<()> {
        ...
        self.peer_credit
            .peer_buf_alloc
            .store(header.buf_alloc, Ordering::Release);
        self.update_peer_fwd_cnt(header.fwd_cnt, tx_cnt);

        let mut rx = self.rx_state.disable_irq().lock();
        let packet_size = packet.payload().len() as u32;
        ...
        if rx.pending_packets.len() >= MAX_PENDING_PACKETS {
            ...
            let rst_packet = FrameVsockPacket::rst(self.local_addr(), self.peer_addr());
            let vcpu_id = self.select_vcpu();
            let _ = backend::send_packet(vcpu_id, backend::PacketCarrier::from_owned(rst_packet));
    ...
    fn send_credit_update_internal(&self, rx: &mut RxState, buf_alloc: u32) {
        ...
        let packet = FrameVsockPacket::credit_update(
            self.local_addr(),
            self.peer_addr(),
            buf_alloc,
            fwd_cnt as u32,
        ...
        let vcpu_id = self.select_vcpu();
        let _ = backend::send_packet(vcpu_id, backend::PacketCarrier::from_owned(packet));
    }
}
