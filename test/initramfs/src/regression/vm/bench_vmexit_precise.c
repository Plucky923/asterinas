...
static int setup_vm(int *kvm_fd, int *vm_fd, int *vcpu_fd, struct kvm_run **run,
		    void **mem, size_t *run_size)
{
	...
	*mem = mmap(NULL, GUEST_MEM_SIZE, PROT_READ | PROT_WRITE,
	...
	gdt[1] =
		0x00cf9a000000ffffULL; /* Code: base=0, limit=4G, DPL=0, exec/read */
	gdt[2] =
		0x00cf92000000ffffULL; /* Data: base=0, limit=4G, DPL=0, read/write */
	...
	*run = mmap(NULL, *run_size, PROT_READ | PROT_WRITE, MAP_SHARED,
		    *vcpu_fd, 0);
	if (*run == MAP_FAILED) {
		perror("mmap vcpu");
		return -1;
...
static void cleanup_vm(int kvm_fd, int vm_fd, int vcpu_fd, struct kvm_run *run,
		       void *mem, size_t run_size)
{
	munmap(run, run_size);
...
static void bench_differential(void)
{
	...
	if (setup_vm(&kvm_fd, &vm_fd, &vcpu_fd, &run, &mem, &run_size) < 0)
		return;
	...
	uint8_t guest_code[] = {
		...
		0x49, 0xff,
		0xc9, /* dec r9 (use r9 as counter in case ecx is clobbered) */
	...
	for (int i = 0; i < ITERATIONS; i++) {
		if (times_large[i] > times_small[i]) {
			diff_samples[i] = (times_large[i] - times_small[i]) /
					  (N_large - N_small);
		} else {
			diff_samples[i] = 0;
		}
	...
	print_stats("Per VM-EXIT+VM-ENTRY (differential):", diff_samples,
		    ITERATIONS);
