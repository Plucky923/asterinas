...
impl MountNamespace {
    ...
    /// Initializes the singleton initial mount namespace with a caller-provided root filesystem.
    pub(in crate::fs) fn init_singleton_with_root(rootfs: Arc<dyn FileSystem>) -> Result<()> {
        let singleton = init_singleton();
        if singleton.get().is_some() {
            return_errno_with_message!(
                Errno::EBUSY,
                "the initial mount namespace is already initialized"
            );
        }
...
fn init_singleton() -> &'static Once<Arc<MountNamespace>> {
