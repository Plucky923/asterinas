...
use aster_framevsock::{
    create_credit_request, create_credit_update, create_data_packet_with_credit, create_rst,
    create_shutdown,
    flow_control::{DEFAULT_BUF_ALLOC, MAX_PENDING_PACKETS, MAX_PKT_BUF_SIZE},
    trace, ConnectionId, DataPacket, SHUTDOWN_FLAG_BOTH, SHUTDOWN_FLAG_RECV, SHUTDOWN_FLAG_SEND,
};
...
impl Connected {
    ...
    pub fn try_send(
        ...
        if self.local_shutdown.load(Ordering::Acquire)
            ...
            || self.connection_reset.load(Ordering::Acquire)
        {
    ...
    pub fn on_shutdown_received(&self, flags: u32) -> Result<()> {
    ...
    fn check_io_events(&self) -> IoEvents {
        ...
        if peer_send_shutdown {
            events |= IoEvents::RDHUP;
        }

        // Writable if we can send (check peer credit)
        let available = self.available_credit(tx.tx_cnt);
        if available > 0 && !local_shutdown && !peer_recv_shutdown && !tx.tx_blocked_on_queue {
            events |= IoEvents::OUT;
        }

        if peer_send_shutdown && peer_recv_shutdown {
            events |= IoEvents::HUP;
        }
