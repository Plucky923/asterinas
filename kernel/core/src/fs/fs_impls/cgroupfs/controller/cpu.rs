...
impl CpuController {
    ...
    fn read_cpu_stat(&self, printer: &mut VmPrinter) -> Result<usize> {
        fn counter_usec(jiffies: u64) -> u64 {
            u64::try_from(Jiffies::new(jiffies).as_duration().as_micros()).unwrap_or(u64::MAX)
        }

        let (user_ticks, system_ticks) = self.task_group.cpu_time_ticks();
        let user_usec = counter_usec(user_ticks);
        let system_usec = counter_usec(system_ticks);
        writeln!(
            printer,
            "usage_usec {}",
...
impl super::SubController<CpuController> {
    ...
    /// Accounts one [`Jiffies`] of CPU time through this cgroup's scheduler hierarchy.
    fn account(&self, stat_kind: CpuStatKind) {
        let cpu = CpuId::current_racy();
        let task_group = &self.inner.as_ref().unwrap().task_group;
        match stat_kind {
            CpuStatKind::User => task_group.account_user_tick(cpu),
            CpuStatKind::System => task_group.account_system_tick(cpu),
        }
    }
}
...
impl super::Controller {
    ...
    fn charge_cpu_time<G: AsAtomicModeGuard + ?Sized>(&self, guard: &G, stat_kind: CpuStatKind) {
        self.cpu.read_with(guard).account(stat_kind);
    }
