...
fn current_domain() -> DomainId {
    crate::task::current_frame_vcpu_id()
        .map(|frame_vcpu_id| DomainId::Guest(frame_vcpu_id.vm_id()))
...
mod tests {
    ...
    fn metadata(id: RRefId) -> Option<RRefMetadata> {
        get_registry().metadata(id)
    }

    fn contains(id: RRefId) -> bool {
        metadata(id).is_some()
    }
    ...
    fn metadata_tracks_owner_transfer_and_borrow_state() {
        ...
        assert!(registry.transfer(id, DomainId::Host, DomainId::Guest(7)));
        let metadata = registry.metadata(id).unwrap();
        assert_eq!(metadata.owner(), DomainId::Guest(7));
        assert_eq!(metadata.state(), RRefState::Live);
    ...
    fn transfer_rejects_wrong_owner_and_borrowed_rref() {
        ...
        assert!(!registry.transfer(id, DomainId::Guest(1), DomainId::Guest(2)));
        assert_eq!(registry.metadata(id).unwrap().owner(), DomainId::Host);

        assert!(registry.begin_borrow(id));
        assert!(!registry.transfer(id, DomainId::Host, DomainId::Guest(1)));
        assert_eq!(registry.metadata(id).unwrap().owner(), DomainId::Host);
    ...
    fn duplicate_registration_is_rejected_without_overwriting_metadata() {
        ...
        assert!(!registry.try_register(RRefMetadata::new::<u64>(id, DomainId::Guest(1),)));
    ...
    fn invalid_registration_is_rejected() {
        ...
        assert!(registry.try_unregister(RREF_ID_INVALID).is_none());
    }
    ...
    fn reclaim_domain_marks_only_owned_rrefs() {
        ...
        registry.register(RRefMetadata::new::<u64>(2, DomainId::Guest(1)));
        registry.register(RRefMetadata::new::<u64>(3, DomainId::Guest(2)));

        let reclaimed = registry.reclaim_domain(DomainId::Guest(1));
    ...
    fn reclaim_domain_waits_for_active_borrows() {
        let registry = RRefRegistry::new();
        registry.register(RRefMetadata::new::<u64>(1, DomainId::Guest(1)));
    ...
    fn rref_token_transfer_updates_inline_and_registry_owner() {
        ...
        let rref = match rref.try_transfer_to(DomainId::Guest(9)) {
            Ok(rref) => rref,
            Err(_) => panic!("host-owned RRef transfer should succeed"),
        };

        assert_eq!(rref.owner(), DomainId::Guest(9));
        assert_eq!(metadata(id).unwrap().owner(), DomainId::Guest(9));

        drop(host_domain_guard);
        let guest_domain_guard = enter_domain(DomainId::Guest(9));
        let rref = match rref.try_transfer_to(DomainId::Host) {
            Ok(rref) => rref,
            Err(_) => panic!("guest-owned RRef transfer back should succeed"),
        };
        assert_eq!(metadata(id).unwrap().owner(), DomainId::Host);

        drop(guest_domain_guard);
    ...
    fn rref_failed_transfer_returns_original_token() {
        init();
        let host_domain_guard = enter_domain(DomainId::Host);
        let rref = RRef::new_with_owner(0x77u64, DomainId::Guest(10));
        let id = rref.id();
        ...
        assert_eq!(rref.owner(), DomainId::Guest(10));
        assert_eq!(metadata(id).unwrap().owner(), DomainId::Guest(10));

        drop(host_domain_guard);
        let guest_domain_guard = enter_domain(DomainId::Guest(10));
        let rref = match rref.try_transfer_to(DomainId::Host) {
            Ok(rref) => rref,
    ...
    fn rref_borrow_blocked_transfer_returns_original_token() {
        ...
        let rref = match rref.try_transfer_to(DomainId::Guest(13)) {
        ...
        assert_eq!(metadata(id).unwrap().owner(), DomainId::Guest(13));

        drop(host_domain_guard);
        let _guest_domain_guard = enter_domain(DomainId::Guest(13));
        drop(rref);
        assert!(!contains(id));
    }
    ...
    fn rref_try_into_inner_unregisters_without_dropping_value() {
        ...
        let _host_domain_guard = enter_domain(DomainId::Host);
        let rref = RRef::new_with_owner(DropProbe(drop_count.clone()), DomainId::Host);
        let id = rref.id();
    ...
    fn rref_drop_unregisters_metadata_even_from_non_owner_domain() {
        ...
        let drop_count = Arc::new(AtomicUsize::new(0));
        let _host_domain_guard = enter_domain(DomainId::Host);
        let rref = RRef::new_with_owner(DropProbe(drop_count.clone()), DomainId::Guest(12));
        let id = rref.id();

        assert_eq!(metadata(id).unwrap().owner(), DomainId::Guest(12));
        drop(rref);
    ...
    fn rref_try_into_inner_rejects_non_owner_without_unregistering() {
        init();
        let host_domain_guard = enter_domain(DomainId::Host);
        let rref = RRef::new_with_owner(0x99u64, DomainId::Guest(11));
        let id = rref.id();
        ...
        assert_eq!(rref.owner(), DomainId::Guest(11));
        assert_eq!(metadata(id).unwrap().owner(), DomainId::Guest(11));

        drop(host_domain_guard);
        let guest_domain_guard = enter_domain(DomainId::Guest(11));
        let rref = match rref.try_transfer_to(DomainId::Host) {
            Ok(rref) => rref,
