...
/*
 * Unix Domain Socket Throughput Benchmark
 *
 * This program measures Unix domain socket throughput using various buffer sizes.
 * Similar to pipe throughput test, it outputs results in Gbits/s.
 */
...
static int run_throughput_test(size_t buf_size, double *throughput_gbps)
{
	...
	if (pid == 0) {
		...
		while (bytes_received < total_to_send) {
			ssize_t n = read(sv[1], rx_buffer, buf_size);
			if (n <= 0)
				break;
			bytes_received += n;
		}
	...
	while (bytes_sent < total_to_send) {
		...
		ssize_t ret = write(sv[0], tx_buffer, to_send);
		if (ret < 0) {
			perror("write");
			close(sv[0]);
			close(result_fd[0]);
...
FN_TEST(unix_socket_throughput_test)
{
	// Initialize buffer
	...
	printf("\n");
	printf("========================================\n");
	printf(" Unix Socket Throughput Benchmark\n");
	printf("========================================\n");
