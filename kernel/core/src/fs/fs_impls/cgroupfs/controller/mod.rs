...
impl<T: SubControlStatic> SubController<T> {
    fn new(parent_controller: Option<&Controller>) -> Self {
        ...
        let inner = if is_active || matches!(T::type_(), SubCtrlType::Cpu | SubCtrlType::CpuSet) {
...
impl Controller {
    ...
    fn update_sub_controllers_for_descents(
        ...
        fn update_sub_controller_for_one_child(
            ...
            match ctrl_type {
                SubCtrlType::CpuSet => {
                    let mut new_controller: SubController<CpuSetController> =
                        SubController::new(Some(parent_controller));
                    {
                        let guard = child_node.controller().cpuset.read();
                        let previous_controller = guard.get();
                        let previous_cpuset_controller =
                            previous_controller.inner.as_ref().unwrap();
                        let new_cpuset_controller = new_controller.inner.as_mut().unwrap();
                        new_cpuset_controller.inherit_state_from(
                            previous_cpuset_controller,
                            parent_controller
                                .active_set()
                                .contains_type(SubCtrlType::CpuSet),
                        );
                    }
                    child_node
                        .controller()
