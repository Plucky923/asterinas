...
use super::super::{VMADDR_CID_ANY, VMADDR_CID_LOCAL, VMADDR_PORT_ANY, VsockSocketAddr};
use crate::{
    error::{Errno, Result},
...
impl PortTable {
    ...
    fn bind_listener_child(&mut self, port: u32) {
        *self.usage.entry(port).or_insert(0) += 1;
    }
...
impl BoundPort {
    ...
    /// Creates an additional lease owned by a stream accepted from a listener.
    pub(in crate::net::socket::vsock) fn new_listener_child(local_addr: VsockSocketAddr) -> Self {
        let port_table = PORT_TABLE.call_once(|| SpinLock::new(PortTable::new()));
        port_table.lock().bind_listener_child(local_addr.port);
        Self { local_addr }
    }

    /// Returns the local address described by this lease.
    pub(in crate::net::socket::vsock) const fn local_addr(&self) -> VsockSocketAddr {
        self.local_addr
    }
}
