...
//! Transitional host-side state for a stream socket that is connecting.

use core::sync::atomic::{AtomicBool, AtomicU32, Ordering};

use framev_sock_common::ConnectionId;
...
pub struct Connecting {
    ...
    recycle_port_on_drop: AtomicBool,
    pollee: Pollee,
...
impl Connecting {
    pub fn new(peer_addr: FrameVsockAddr, local_addr: FrameVsockAddr) -> Self {
        Self {
            ...
            recycle_port_on_drop: AtomicBool::new(true),
            pollee: Pollee::new(),
    ...
    pub fn preserve_local_port(&self) {
        self.recycle_port_on_drop.store(false, Ordering::Release);
    }
...
impl Drop for Connecting {
    fn drop(&mut self) {
        ...
        if !self.is_connected.load(Ordering::Relaxed)
            && self.recycle_port_on_drop.load(Ordering::Relaxed)
            && let Some(vsockspace) = FRAME_VSOCK_GLOBAL.get()
        {
