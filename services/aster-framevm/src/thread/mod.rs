...
use crate::{
    net,
    prelude::*,
    process::{pid_table, signal::constants::SIGKILL},
...
fn shutdown_handler() {
    ...
    let processes = pid_table::pid_table_mut()
        .iter_processes()
        .collect::<Vec<_>>();
    for process in processes {
        process.enqueue_signal(Box::new(
            crate::process::signal::signals::kernel::KernelSignal::new(SIGKILL),
        ));
    }
}
