...
impl SectionMemory {
    pub fn protect_final_permissions(&self) -> Result<()> {
...
pub fn caculate_section_size(elf_file: &ElfFile) -> Result<(usize, usize, usize)> {
    ...
    for sh in elf_file.section_iter() {
        ...
        match bucket {
            SectionMemoryType::Text => {
                exec_cursor = align_up_checked(exec_cursor, align)?;
                exec_cursor = exec_cursor
                    .checked_add(size)
                    .ok_or_else(|| invalid_args("executable section size overflows"))?;
            }
            SectionMemoryType::RoData => {
                ro_cursor = align_up_checked(ro_cursor, align)?;
                ro_cursor = ro_cursor
                    .checked_add(size)
                    .ok_or_else(|| invalid_args("read-only section size overflows"))?;
            }
            SectionMemoryType::RwData => {
                rw_cursor = align_up_checked(rw_cursor, align)?;
                rw_cursor = rw_cursor
                    .checked_add(size)
                    .ok_or_else(|| invalid_args("writable section size overflows"))?;
            }
        }
    }

    Ok((exec_cursor, ro_cursor, rw_cursor))
}
...
pub fn alloc_section_memory(
    ...
    let exec_pages = bytes_to_pages(exec_bytes)?;
    let ro_pages = bytes_to_pages(ro_bytes)?;
    let rw_pages = bytes_to_pages(rw_bytes)?;
...
fn alloc_pages(pages: usize, flags: PageFlags) -> Result<Option<KVirtArea>> {
    ...
    let prop = section_prop(flags);

    // area_size 需要是字节数，map_offset 应该从 0 开始
    let area_size = pages.checked_mul(PAGE_SIZE).ok_or(crate::Error::Overflow)?;
    let kvirt_area = KVirtArea::map_module_frames(area_size, 0, segment.into_iter(), prop)?;
    Ok(Some(kvirt_area))
}
...
fn bytes_to_pages(size: usize) -> Result<usize> {
    if size == 0 {
        return Ok(0);
    }
    let aligned_size = align_up_checked(size, PAGE_SIZE)?;
    Ok(aligned_size / PAGE_SIZE)
}
