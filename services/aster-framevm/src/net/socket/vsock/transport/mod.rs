...
use framev_sock_common::{
    ...
    FrameVsockSendError, HOST_CID, MAX_PACKET_PAYLOAD_LEN, VsockOp,
};
...
static TRANSPORT_GENERATION: AtomicU64 = AtomicU64::new(1);
// This upper-socket copy creates the owned packet allocation. FrameVisor moves
// that allocation unchanged, so it is intentionally separate from bridge-copy
// accounting.
static SOCKET_EDGE_BYTES: AtomicU64 = AtomicU64::new(0);
...
/// Returns the generation observed by newly created socket flows.
pub(super) fn current_transport_generation() -> u64 {
    TRANSPORT_GENERATION.load(Ordering::Acquire)
}

/// Invalidates old-generation flow state while retaining listener objects.
pub(super) fn handle_transport_reset(generation: u64) {
    let previous = TRANSPORT_GENERATION.swap(generation, Ordering::AcqRel);
    if generation <= previous {
        return;
    }

    inbound_packets().lock().packets_by_flow.clear();
    flow_affinity().lock().clear();
    let pollees = flow_pollees().lock().values().cloned().collect::<Vec<_>>();
    let wait_queues = flow_wait_queues()
        .lock()
        .values()
        .cloned()
        .collect::<Vec<_>>();
    for pollee in pollees {
        pollee.notify(IoEvents::ERR | IoEvents::HUP);
    }
...
pub(super) fn build_rw_packets(
    ...
    while offset < sendable_len {
        ...
        let packet_len_u64 = u64::try_from(packet_len).unwrap_or(u64::MAX);
        let _ = SOCKET_EDGE_BYTES.fetch_update(Ordering::Relaxed, Ordering::Relaxed, |current| {
            Some(current.saturating_add(packet_len_u64))
        });
        packets.push(packet);
        offset += packet_len;
    }
...
fn submit_packet(
    ...
    framev_sock_frontend::submit_packet(queue_id, packet)
        .map_err(|(error, packet)| (TransportError::from(error), packet))?;
    if removes_flow {
...
fn select_queue(packet: &FrameVsockPacket) -> usize {
    ...
    let key = FrameVsockFlowKey::from_packet(packet);
    let preferred_queue = key.preferred_queue(queue_count);
...
impl From<FrameVsockSendError> for TransportError {
    fn from(error: FrameVsockSendError) -> Self {
        match error {
            FrameVsockSendError::InvalidPacket => Self::InvalidPacket,
            FrameVsockSendError::DestinationUnavailable => Self::DestinationUnavailable,
            FrameVsockSendError::ReceiverFull => Self::ReceiverFull,
            FrameVsockSendError::Stopped => Self::Stopped,
        }
    }
}
...
impl TransportError {
    fn into_error(self) -> Error {
        match self {
            Self::InvalidPacket => Error::with_message(Errno::EINVAL, "invalid socket packet"),
            Self::DestinationUnavailable => {
                Error::with_message(Errno::ECONNREFUSED, "socket destination is unavailable")
            }
            Self::ReceiverFull => Error::with_message(Errno::EAGAIN, "socket queue is full"),
            Self::Stopped => Error::with_message(Errno::ECONNRESET, "socket transport stopped"),
        }
    }
}
