...
impl LocalWorkerPool {
    ...
    fn add_worker(&self) {
        if self
            .parent
            .upgrade()
            .is_none_or(|worker_pool| worker_pool.is_stopping())
        {
            return;
        }
...
impl WorkerPool {
    ...
    /// Starts worker-pool shutdown without dropping the pool's static owner.
    pub fn shutdown(&self) {
        if self.is_stopping.swap(true, Ordering::AcqRel) {
            return;
        }
        self.monitor.stop();
        for local_pool in &self.local_pools {
...
impl Monitor {
    ...
    fn run_monitor_loop(self: &Arc<Self>) {
        ...
        loop {
            ...
            let _ = self.sleep_queue.wait_until_or_timeout(
                || self.is_stopping.load(Ordering::Acquire).then_some(()),
                &sleep_duration,
            );
        }
    }
