...
impl FairClassRq {
    ...
    pub(super) fn try_dequeue_frame_sched_group(
        &mut self,
        state: &Arc<frame_group::FrameSchedEntityState>,
    ) -> bool {
        let task_group = state.task_group();
    ...
    pub(super) fn enqueue_frame_sched_group(
        ...
        if self
            ...
            .is_some_and(|self_group| Arc::ptr_eq(&task_group, &self_group))
        {
            return self.enqueue_frame_sched_group_direct(state);
        }

        let (was_empty, was_queued) = {
            let mut leaf_rq = task_group.fair_queue(self.cpu).disable_irq().lock();
            let was_empty = leaf_rq.is_empty();
            let was_queued = leaf_rq.has_entity(state.fair_attr().id);
            leaf_rq.enqueue_frame_sched_group_direct(state);
        ...
        if was_empty && let Some(self_group) = self.task_group() {
            let mut child_group = task_group;
    ...
    fn enqueue_frame_sched_group_direct(
        &mut self,
        state: Arc<frame_group::FrameSchedEntityState>,
    ) -> bool {
        let fair_attr = state.fair_attr();
...
mod tests {
    use aster_framevisor::{
        DEFAULT_FRAMEVM_SHARE, FrameSchedGroup, FrameVcpuId, irq::InterruptHandler,
    };
    use ostd::{cpu::CpuSet, prelude::ktest};

    use super::*;
    use crate::sched::sched_class::{self, frame_group::FrameSchedEntityState};

    fn frame_group_state(
        vm_id: u32,
        vcpu_index: usize,
        task_group: Arc<TaskGroup>,
    ) -> Arc<FrameSchedEntityState> {
        let id = FrameVcpuId::new(vm_id.into(), vcpu_index);
        ...
        Arc::new(FrameSchedEntityState::new(
            &group,
            task_group,
            CpuSet::new_full(),
        ))
    ...
    fn count_framevm_group_picks(first_share: u32, second_share: u32, rounds: u32) -> (u32, u32) {
        let cpu = CpuId::bsp();
        let root = TaskGroup::new_root(1);
        let first_group = sched_class::create_framevm_task_group(&root, first_share);
        let second_group = sched_class::create_framevm_task_group(&root, second_share);
        let first = frame_group_state(1, 0, first_group.clone());
        let second = frame_group_state(2, 0, second_group.clone());
        let runtime_delta = min_period_clocks() * 2;
        ...
        let mut second_picks = 0;
        let mut root_rq = root.fair_queue(cpu).disable_irq().lock();

        assert!(root_rq.enqueue_frame_sched_group(first.clone()));
        assert!(root_rq.enqueue_frame_sched_group(second));
        assert!(!root_rq.enqueue_frame_sched_group(first));
        for _ in 0..rounds {
            let Some(FairPick::FrameSchedGroup(current)) = root_rq.pick_next_fair() else {
                panic!("a runnable FrameVM group must yield its vCPU entity");
            };
            if Arc::ptr_eq(current.task_group(), &first_group) {
                first_picks += 1;
            } else if Arc::ptr_eq(current.task_group(), &second_group) {
                second_picks += 1;
            } else {
                panic!("picked vCPU must belong to one of the competing FrameVMs");
            }

            root_rq.update_current_frame_group(&current, &runtime, UpdateFlags::Tick);
            root_rq.enqueue_frame_sched_group(current);
        }

        (first_picks, second_picks)
    }

    #[ktest]
    fn equal_share_framevms_both_progress() {
        let (first_picks, second_picks) =
            count_framevm_group_picks(DEFAULT_FRAMEVM_SHARE, DEFAULT_FRAMEVM_SHARE, 64);

        assert!(first_picks > 0);
        assert!(second_picks > 0);
        assert!(
            first_picks.abs_diff(second_picks) <= 1,
            "equal-share FrameVMs should receive balanced service: first={first_picks}, second={second_picks}"
        );
    }

    #[ktest]
    fn unequal_share_framevms_preserve_progress_and_weight() {
        let (low_picks, high_picks) =
            count_framevm_group_picks(DEFAULT_FRAMEVM_SHARE / 4, DEFAULT_FRAMEVM_SHARE, 80);

        assert!(low_picks > 0);
        assert!(high_picks > 0);
        assert!(
            high_picks >= low_picks.saturating_mul(2),
            "higher-share FrameVM should receive proportionally more service: low={low_picks}, high={high_picks}"
        );
    }

    #[ktest]
    fn frame_vcpu_entities_share_one_hidden_task_group() {
        let cpu = CpuId::bsp();
        let root = TaskGroup::new_root(1);
        let framevm_group = TaskGroup::new_child(&root, DEFAULT_CGROUP_WEIGHT);
        let first = frame_group_state(1, 0, framevm_group.clone());
        let second = frame_group_state(1, 1, framevm_group.clone());
        let mut root_rq = root.fair_queue(cpu).disable_irq().lock();

        assert!(root_rq.enqueue_frame_sched_group(first.clone()));
        assert!(root_rq.enqueue_frame_sched_group(second));
        assert!(!root_rq.enqueue_frame_sched_group(first));
        assert_eq!(root_rq.entities.len(), 1);
        assert_eq!(framevm_group.fair_queue(cpu).lock().entities.len(), 2);
    }

    #[ktest]
    fn nested_pick_preserves_frame_vcpu_entity() {
        let cpu = CpuId::bsp();
        let root = TaskGroup::new_root(1);
        let parent = TaskGroup::new_child(&root, DEFAULT_CGROUP_WEIGHT);
        let framevm_group = TaskGroup::new_child(&parent, DEFAULT_CGROUP_WEIGHT);
        let state = frame_group_state(7, 0, framevm_group);
        let mut root_rq = root.fair_queue(cpu).disable_irq().lock();
        root_rq.enqueue_frame_sched_group(state.clone());

        let Some(FairPick::FrameSchedGroup(picked)) = root_rq.pick_next_fair() else {
            panic!("nested FrameVM group must yield its vCPU entity");
        };
        assert!(Arc::ptr_eq(&picked, &state));
    }
}
...
impl FairClassRq {
    pub(super) fn update_current_frame_group(
        ...
        let mut should_preempt = if Arc::ptr_eq(task_group, &self_group) {
            self.update_current_entity(state.fair_attr(), rt, flags)
        } else {
            task_group
                .fair_queue(self.cpu)
                .disable_irq()
                .lock()
                .update_current_entity(state.fair_attr(), rt, flags)
        };
        ...
        while let Some(parent_group) = current_group.parent() {
            ...
            if Arc::ptr_eq(&parent_group, &self_group) {
                ...
                should_preempt |= parent_group
                    .fair_queue(self.cpu)
                    .disable_irq()
                    .lock()
                    .update_current_entity(group_attr, rt, flags);
            }
            current_group = parent_group;
        }
        // Host TaskGroup accounting remains on the ordinary hierarchical path.
        // FrameVM timer delivery is coalesced and consumed by its interrupt
        // handler; feeding the tick back into that source here would never
        // drain the pending timer state.
