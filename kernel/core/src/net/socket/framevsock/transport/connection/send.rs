...
use framev_sock_common::{FrameVsockPacket, flow_control::MAX_PKT_BUF_SIZE};
...
impl Connected {
    ...
    /// 2. Create FrameVsockPacket with the Vec
    ...
    pub fn try_send(
        ...
        if let Some(packet) = pending_packet.take() {
            ...
            let pending_len = packet.payload().len();
            ...
            if let Err(packet) =
                backend::send_packet(vcpu_id, backend::PacketCarrier::from_packet(packet))
            {
                let mut tx = self.tx_state.disable_irq().lock();
                ...
                *pending_packet = Some(packet.into_packet());
                drop(tx);
        ...
        let mut data = alloc::vec![0u8; to_send];
        ...
        .map_err(|_| Error::with_message(Errno::EMSGSIZE, "invalid FrameVsock payload size"))?;

        let vcpu_id = self.select_vcpu();
        if let Err(packet) =
            backend::send_packet(vcpu_id, backend::PacketCarrier::from_owned(packet))
        {
            let mut tx = self.tx_state.disable_irq().lock();
    ...
    fn send_credit_request(&self) -> bool {
        let packet = FrameVsockPacket::credit_request_with_credit(
            self.local_addr(),
            self.peer_addr(),
        ...
        let vcpu_id = self.select_vcpu();
        backend::send_packet(vcpu_id, backend::PacketCarrier::from_owned(packet)).is_ok()
    }
}
