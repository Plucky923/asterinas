...
/// Stable frontend-facing reason for a synchronous packet submission failure.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum FrameVsockSendError {
    /// Packet authority or queue placement is invalid.
    InvalidPacket,
    /// The requested destination is denied, absent, or stale.
    DestinationUnavailable,
    /// The destination receive queue has no capacity.
    ReceiverFull,
    /// The transport stopped or reset during submission.
    Stopped,
}
...
impl FrameVsockFlowKey {
    ...
    /// Selects the preferred queue for this flow.
    ///
    /// Both endpoints use this direction-independent mapping so that queue
    /// backpressure is reported to the connection that actually owns the
    /// selected queue.
    pub const fn preferred_queue(&self, queue_count: usize) -> usize {
        let queue_count = if queue_count == 0 { 1 } else { queue_count };
        let hash = self
            .first
            .cid
            .wrapping_add(self.first.port as u64)
            .wrapping_add(self.second.cid.rotate_left(17))
            .wrapping_add((self.second.port as u64).rotate_left(7));
...
impl FrameVsockPacket {
    ...
    /// Clones a packet without turning an allocation failure into a panic.
    ///
    /// FrameVisor uses this when copying a service-owned packet into the
    /// Host-managed exchange heap. The original packet remains owned by the
    /// service if the Host copy cannot be created.
    pub fn try_clone(&self) -> Option<Self> {
        let mut payload = Vec::new();
        payload.try_reserve(self.payload.len()).ok()?;
...
pub mod flow_control {
    ...
    pub const fn should_send_credit_update(
        ...
        // A low free-space watermark alone must not cause the same credit
        // update to be requested repeatedly before the receiver advances.
        if consumed_since_last == 0 {
            return false;
        }
...
mod tests {
    ...
    #[test]
    fn flow_key_selects_the_same_queue_across_directions() {
        let forward = FrameVsockFlowKey::new(src_addr(), dst_addr());
        let reverse = FrameVsockFlowKey::new(dst_addr(), src_addr());

        assert_eq!(forward.preferred_queue(4), reverse.preferred_queue(4));
        assert_eq!(forward.preferred_queue(0), 0);
    }
    ...
    #[test]
    fn credit_update_requires_forward_progress() {
        use crate::flow_control::should_send_credit_update;

        assert!(!should_send_credit_update(4096, 4096, 100, 100));
        assert!(should_send_credit_update(4096, 4096, 101, 100));
        assert!(should_send_credit_update(4096, 0, 101, 100));
    }

    #[test]
    fn credit_update_preserves_counter_wraparound() {
        use crate::flow_control::should_send_credit_update;

        assert!(should_send_credit_update(4096, 4096, 3, u32::MAX - 1023));
        assert!(!should_send_credit_update(4096, 4096, 7, 7));
    }
    ...
    #[test]
    fn packet_exchange_charge_includes_payload_capacity() {
        let mut payload = Vec::with_capacity(128);
        payload.extend_from_slice(&[1, 2, 3]);
        let packet = FrameVsockPacket::rw(src_addr(), dst_addr(), payload, 4096, 0).unwrap();

        assert!(packet.allocation_size() >= size_of::<FrameVsockPacket>() + 128);
    }

    #[test]
    fn packet_try_clone_preserves_header_and_payload() {
        let packet =
            FrameVsockPacket::rw(src_addr(), dst_addr(), Vec::from([1_u8, 2, 3]), 4096, 0).unwrap();
        let copy = packet.try_clone().unwrap();
        assert_eq!(copy.header(), packet.header());
        assert_eq!(copy.payload(), packet.payload());
        assert_ne!(copy.payload().as_ptr(), packet.payload().as_ptr());
    }
}
